...
<form
  id="dropzone"
  class="shadow-2xl border-dashed bg-gray-400 border-2 border-orange-600 redounded-lg aspect-video w-full flex justify-center items-center flex-col"
  action="https://api.cloudinary.com/v1_1/ddeun9v2x/image/upload"
>
  {#if $imageStatus === ImageStatus.READY}
    <button
      class="pointer-events-none bg-blue-600 rounded-xl
    text-bold text-white text-xl px-6 py-4"
    >
      Upload files
    </button>
    <strong class="text-lg mt-4 text-orange-600">Or drop a file</strong>
  {:else if $imageStatus === ImageStatus.UPLOADING}
    <strong class="text-lg mt-4 text-orange-600">Uploading files</strong>
  {/if}
</form>
